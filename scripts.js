...
//alert($(window).width());
if( $(window).width()<640 ){
	$('meta[name=viewport]').attr('content','width=device-width initial-scale=0.7, maximum-scale=0.7, minimum-scale=0.7, user-scalable=no, minimal-ui=1');
}
	...
	$.getJSON( urlFile, function(data) {
		...
		for(var i = 0; i < dataSource.length; ++i) {
			...
			for(var prop in json) {
				...
				} else if( !isNaN(parseFloat(json[prop])) ){
